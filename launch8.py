...
class SimpleTableTennisLauncher:
    ...
    def parameters_to_velocity_spin(self, phi, theta, rpm_tl, rpm_tr, rpm_bc):
     ...
     spin_yaw = (rpm_tr - rpm_tl) * rpm_to_rads * 0.9 
    
     spin_x = 0.0
     spin_y = spin_yaw 
     spin_z = -spin_pitch """
     spin_yaw = (rpm_bc - (rpm_tl + rpm_tr)/2) * rpm_to_rads * 0.7  
     ...
     print(f"Wheel RPMs: TL={rpm_tl}, TR={rpm_tr}, BC={rpm_bc}")
     print(f"Calculated spins: Roll={spin_x:.2f}, Sidespin={spin_y:.2f}, Topspin={spin_z:.2f} rad/s")
    ...
    """
    def apply_magnus_force(self, k=1e-5, body_id=None):
     qveladr = self.model.jnt_dofadr[self.ball_joint_id]
     v = self.data.qvel[qveladr:qveladr + 3].copy()   
     omega = self.data.qvel[qveladr + 3:qveladr + 6].copy()
    
    # Standard Magnus force calculation
     F_magnus = k * np.cross(omega, v)
    
    # COORDINATE SYSTEM CORRECTION: Manually flip the Y-component
     F_magnus = -F_magnus  # Force opposite Y direction
    
     if body_id is None:
        body_id = int(self.model.jnt_bodyid[self.ball_joint_id])
    
    # Apply only the corrected force (no torque for now)
     self.data.xfrc_applied[body_id, :3] += F_magnus """
    ...
    # Try different approach - check spin direction and force Y accordingly
     if omega[1] > 0:  # Positive sidespin
        F_magnus[1] = -abs(F_magnus[1])  # Force negative Y
     else:  # Negative sidespin  
        F_magnus[1] = +abs(F_magnus[1])  # Force positive Y
    ...
    """
    def apply_magnus_force(self, k=1.5e-5, body_id=None):
     qveladr = self.model.jnt_dofadr[self.ball_joint_id]
     v = self.data.qvel[qveladr:qveladr + 3].copy()   
     omega = self.data.qvel[qveladr + 3:qveladr + 6].copy()
     F_magnus = k * np.cross(omega, v)  # ω × v for proper Magnus direction
     if body_id is None:
        body_id = int(self.model.jnt_bodyid[self.ball_joint_id])
     T_magnus = 0.01 * k * np.cross(omega, F_magnus) 
     self.data.xfrc_applied[body_id, :3] += F_magnus      # Linear force
     self.data.xfrc_applied[body_id, 3:6] += T_magnus     # Rotational torque
     """
